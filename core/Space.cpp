...
void Space::updateSystems(double delta)
{
    ...
    _systemGraph.vertexFuncs[State::WHITE] = [&](SystemGraphVertex &v, SystemGraph &) {
        ...
        while (remaining >= 0)
        {
            ...
            // Find currently alive Entities and update System
            ...
            // Process any SPACE_DELETE_ENTITY events generated by previous System::update
            auto events = v.data->popEvents(StreamType::OUTGOING);
            v.data->pushEvents(events, StreamType::OUTGOING);

            std::vector<Event> deleteEntityEvents;
            std::copy_if(events.begin(), events.end(), std::back_inserter(deleteEntityEvents), [](const Event &e){ 
                return e.type == core::event::type::SPACE_DELETE_ENTITY;
            });

            _eventStream.pushEvents(deleteEntityEvents, StreamType::INCOMING);
            _eventStream.processEvents();
